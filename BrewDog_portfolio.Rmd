...
We notice there are two bends (elbows) one at 2 and the other at 6. As the information from this analysis will be used for marketing purpose, a greater number of clusters will be helpful. So, 6 is a suitable number of clusters going forward. Using this knowledge of 6 clusters as the optimal one, we get the desired dendrogram as follows

```{r echo=TRUE}
# Clustered dendrogram
brew_dendrogram <- as.dendrogram(clust)
brew_dendrogram.col <- brew_dendrogram %>%
  set("branches_k_color", k = 6, value =   c("#E69F00", "#56B4E9", "#009E73", "#CC79A7", "#0072B2", "#D55E00")) %>%
...
ggplot(ggd1, theme = theme_minimal()) + 
  labs(x = "Num. observations", y = "Dissimiliraty", title = "Agglomerative, complete linkages, k = 6")
```
...
cluster_data <- data.frame(brewdogData_complete, clusterNo = cutree(clust, k=6))
...
On analysis of the data that has been provided by BrewDog, it is observed that beers can be clustered into six types. Information about these six clusters will help BrewDog to target similar product to consumers.
